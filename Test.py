...
# config variables model_name  = config["model"] weights  = config["weights"] include_top  = config["include_top"] train_path  = config["train_path"] test_path  = config["test_path"] features_path  = config["features_path"] labels_path  = config["labels_path"] test_size  = config["test_size"] results  = config["results"] model_path  = config["model_path"] seed  = config["seed"] classifier_path = config["classifier_path"]
...
# get all the test images paths test_images = os.listdir(test_path) labelss=[] res=[] cnt=0 p = [] i = 0
# loop through each image in the test data for imagePath in paths.list_images("dataset\\test"): i = i+1
#path  = test_path + "/" + image_path print(imagePath)
make = imagePath.split("\\")[-2] print (make) pred=[]
img1  = image.load_img(imagePath, target_size=image_size1) img2  = image.load_img(imagePath, target_size=image_size3) x1  = image.img_to_array(img1)
x1  = np.expand_dims(x1, axis=0) x1  = preprocess_input(x1) feature1  = model1.predict(x1) feature2  = model2.predict(x1) flat1  = feature1.flatten() flat1  = np.expand_dims(flat1, axis=0) preds1  = classifier1.predict(flat1) pred.append(preds1[0]) prediction1 = train_labels[preds1[0]] flat2  = feature2.flatten() flat2  = np.expand_dims(flat2, axis=0) preds2  = classifier2.predict(flat2) pred.append(preds2[0]) prediction2 = train_labels[preds2[0]] x2  = image.img_to_array(img2) x2  = np.expand_dims(x2, axis=0) x2  = preprocess_input(x2) feature3  = model3.predict(x2) flat3  = feature3.flatten() flat3  = np.expand_dims(flat3, axis=0) preds3  = classifier3.predict(flat3) prediction3 = train_labels[preds3[0]] pred.append(preds3[0]) idx, ctr = 0, 1 for i in range(1, len(pred)):             if pred[idx] == pred[i]:
                ctr += 1             else:
                ctr -= 1                 if ctr == 0:                     idx = 1
                    ctr = 1
#fin=np.average(int(pred),axis=0) fin_prediction = train_labels[pred[idx]]
print("pred1",prediction1) print("pred2",prediction2) print("pred3",prediction3) print("final",fin_prediction) if make==fin_prediction: cnt=cnt+1 print (cnt)
...
# key tracker
key = cv2.waitKey(1000) p= paths.list_images("dataset\\test") acc=cnt/i print(acc*100)
